...
def get_parameters():
    ...
    parser.add_argument('--dataset', type=str, default='delhiaq', choices=['metr-la', 'pems-bay', 'pemsd7-m', 'delhiaq'])
...
def data_preparate(args, device):    
    ...
    data_col = pd.read_csv(os.path.join(dataset_path, f'vel_{args.pol}.csv')).shape[0]
    ...
    train, val, test = dataloader.load_data(args.dataset, len_train, len_val, args.pol)
...
def prepare_model(args, blocks, n_vertex):
    ...
    es = earlystopping.EarlyStopping(delta=0.0, 
                                     ...
                                     verbose=True, 
                                     path="STGCN_" + args.dataset + args.pol + ".pt")
...
def test(zscore, loss, model, test_iter, args):
    model.load_state_dict(torch.load("STGCN_" + args.dataset + args.pol + ".pt"))
    model.eval()
